...
describe('Class Schedule Listing Parser', () => {
  it('should throw error when wrong page type is given', async () => {
    ...
    await expect(
      async () => await classScheduleListingExtractor($)
  ...
  it('parses ECE260 correctly', async () => {
    ...
    expect(parsed[0].associatedTerm).toStrictEqual({
      start: '202009',
      end: '202012',
    });
...
describe('Class Schedule Listing Parser (CRN) CSC', () => {
  const paths = [
    ...getScheduleBySubject('202009', 'CSC'),
    ...getScheduleBySubject('202101', 'CSC'),
  ];
