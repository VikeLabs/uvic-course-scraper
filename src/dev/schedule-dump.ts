...
export const scheduleUtil = async (term: string) => {
  const writeCourseScheduleToFS = async (jsonCourse: CourseJSON) => {
    const url = classScheduleListingUrl(term, jsonCourse.subject, jsonCourse.code);
    const res = await got(url);
    if (res.body.search(/No classes were found that meet your search criteria/) === -1) {
      const destDir = `static/schedule/${term}/${jsonCourse.subject}`;
      const destFilePath = `${destDir}/${jsonCourse.subject}_${jsonCourse.code}.html`;
  ...
  const jsonCourses = coursesJSON as CourseJSON[];
  await forEachHelper(jsonCourses, writeCourseScheduleToFS, 10);
